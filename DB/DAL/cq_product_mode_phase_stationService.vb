...
Public Class cq_product_mode_phase_stationService
	Public Function SelectAllByProductModeId(product_mode_id As Integer, validation As Boolean, validity As Boolean) As List(Of Model.cq_product_mode_phase_station)
		Try
			Dim dt As DataTable
			Dim resp As New List(Of FACTS.Model.cq_product_mode_phase_station)
			Dim respItem As FACTS.Model.cq_product_mode_phase_station
			...
			dt = SqlHelper.runProcedureWithDataset("proc_cq_phase_station_SelectByProdModeId", parameters, "dt").Tables("dt")
		...
		Catch ex As Exception
			Throw New Exception("cq_product_mode_phase_station.DAL.SelectAllByProductModeId()::" & ex.Message)
		End Try
	End Function
	Public Function DataRowToModel(row As DataRow) As FACTS.Model.cq_product_mode_phase_station
		Try
			Dim model As New FACTS.Model.cq_product_mode_phase_station
			...
			If row.Table.Columns.Contains("product_mode_phase_station_id") Then If Not (IsDBNull(row("product_mode_phase_station_id"))) Then model.M_product_mode_phase_station.id = Convert.ToString(row("product_mode_phase_station_id"))
			...
			If row.Table.Columns.Contains("order_idx") Then If Not (IsDBNull(row("order_idx"))) Then model.M_product_mode_phase_station.order_idx = Convert.ToInt32(row("order_idx"))
			...
			If row.Table.Columns.Contains("default_selection") Then If Not (IsDBNull(row("default_selection"))) Then model.M_product_mode_phase_station.default_selection = Convert.ToBoolean(row("default_selection"))
			...
			If row.Table.Columns.Contains("validity") Then If Not (IsDBNull(row("validity"))) Then model.M_product_mode_phase_station.validity = Convert.ToBoolean(row("validity"))
			If row.Table.Columns.Contains("validation") Then If Not (IsDBNull(row("validation"))) Then model.M_product_mode_phase_station.validation = Convert.ToBoolean(row("validation"))

			Return model

		Catch ex As Exception
			Throw New Exception("cq_product_mode_phase_station.DAL.DataRowToModel()::" & ex.Message)
		End Try
	End Function
End Class
