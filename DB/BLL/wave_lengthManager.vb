Public Class wave_lengthManager
	Private _dal As New FACTS.DAL.wave_lengthService
	Public Function Add(model As FACTS.Model.wave_length) As Boolean
		Try
			Return _dal.Add(model)
		Catch ex As Exception
			Throw New Exception("wave_length.BLL.Add()::" & ex.Message)
		End Try
	End Function


	Public Function AddReturnId(model As FACTS.Model.wave_length) As Integer
		Try
			Return _dal.AddReturnId(model)
		Catch ex As Exception
			Throw New Exception("wave_length.BLL.AddReturnId()::" & ex.Message)
		End Try
	End Function


	Public Function Delete(id As Integer) As Boolean
		Try
			Return _dal.Delete(id)
		Catch ex As Exception
			Throw New Exception("wave_length.DAL.Delete()::" & ex.Message)
		End Try
	End Function


	Public Function Update(model As FACTS.Model.wave_length) As Boolean
		Try
			Return _dal.Update(model)
		Catch ex As Exception
			Throw New Exception("wave_length.BLL.Update()::" & ex.Message)
		End Try
	End Function


	Public Function SelectAll() As List(Of FACTS.Model.wave_length)
		Try
			Return _dal.SelectAll()
		Catch ex As Exception
			Throw New Exception("wave_length.BLL.SelectAll()::" & ex.Message)
		End Try
	End Function


	Public Function SelectById(id As Integer) As FACTS.Model.wave_length
		Try
			Return _dal.SelectById(id)
		Catch ex As Exception
			Throw New Exception("wave_length.BLL.SelectById()::" & ex.Message)
		End Try
	End Function
	Public Function SelectByWavelength(wl As String) As FACTS.Model.wave_length
		Try
			Return _dal.SelectByWavelength(wl)
		Catch ex As Exception
			Throw New Exception("wave_length.BLL.SelectByWavelength()::" & ex.Message)
		End Try
	End Function
End Class
